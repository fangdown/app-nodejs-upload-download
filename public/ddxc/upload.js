...
$("#file").change(async function () {
  ...
  const resp = await fetch("http://localhost:5008/api/ddxc_upload/handshake", {
    method: "POST",
    headers: {
...
async function uploadPiece() {
  ...
  if (needs.length === 0) {
    ...
    domControls.setStatus();
    domControls.setLink(
      `http://localhost:5008/ddxc_download/${fileInfo.fileId}${fileInfo.ext}`
    );
    return;
  }
  ...
  const resp = await fetch("http://localhost:5008/api/ddxc_upload", {
    method: "POST",
    body: formData,
  }).then((resp) => resp.json());
  needs = resp.data;
